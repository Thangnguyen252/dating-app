...
export default function MatchScreen({ currentUser, matchedProfile, onClose, onSendMessage }: MatchScreenProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center min-h-screen p-6 text-center bg-primary font-display antialiased text-slate-900 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full min-h-screen">
        <div className="mb-8 text-slate-900/40 animate-pop-in">
          <svg className="w-14 h-14" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zm0 10l-10 5 10 5 10-5-10-5z"/></svg>
        </div>
        
        <div className="animate-pop-in" style={{ animationDelay: '0.1s' }}>
          <h1 className="font-black text-6xl md:text-8xl text-slate-900 tracking-tighter leading-none mb-4 italic drop-shadow-md">
            It&apos;s a Match!
          </h1>
          <p className="text-slate-900/90 text-xl md:text-3xl font-semibold mb-12">
            You and <span className="text-white drop-shadow-sm">{matchedProfile.name}</span> have matched!
          </p>
        </div>

        <div className="relative flex items-center justify-center mb-16 h-48 md:h-64 w-full max-w-2xl animate-pop-in" style={{ animationDelay: '0.2s' }}>
          {/* Current user */}
          <div className="absolute left-1/2 -translate-x-[90%] md:-translate-x-[85%] z-10 animate-float">
            <div className="w-40 h-40 md:w-56 md:h-56 rounded-full border-[6px] md:border-10 border-white shadow-2xl overflow-hidden bg-stone-200 rotate-[-5deg] relative">
              <Image 
                alt="My Profile" 
          ...
          <div className="absolute -translate-x-1/2 z-30 bg-white rounded-full p-4 md:p-6 shadow-2xl scale-110 md:scale-125">
            <svg className="w-10 h-10 md:w-14 md:h-14 text-primary fill-current" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
          </div>
        ...
        <div className="flex flex-col gap-5 w-full max-w-md animate-pop-in" style={{ animationDelay: '0.3s' }}>
          <button 
            onClick={() => onSendMessage(matchedProfile)}
            className="w-full py-5 px-10 bg-white hover:bg-stone-50 text-slate-900 font-black text-xl md:text-2xl rounded-full shadow-2xl hover:shadow-white/30 transition-all transform hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3 uppercase tracking-widest group"
          >
            <MessageCircle className="w-7 h-7 group-hover:scale-110 transition-transform" />
            Send a Message
          </button>
          <button onClick={onClose} className="w-full py-4 px-8 bg-transparent border-4 border-slate-900/30 hover:border-slate-900 text-slate-900 font-extrabold text-lg md:text-xl rounded-full transition-all hover:bg-slate-900/10 uppercase tracking-widest">
            Keep Swiping
          </button>
        </div>
